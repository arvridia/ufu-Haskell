...
triangulo x y z
 ...
 | otherwise = "Nao eh triangulo!"
	where 
		  ehPositivo  = (x >= 0) && (y >= 0) && (z >= 0); ehTriangulo = (x+y > z) && (y+z > x) && (z+x > y)

--Arthur Resende Santos 12011BCC020
--Exercicio 4	  
quadradoMaior :: Float -> Float -> Float
quadradoMaior a b = let quadSoma = (a+b)**2
                        somaQuad = (a**2)+(b**2)
					in if quadSoma >= somaQuad
					      then quadSoma 
					   else somaQuad
...
main = do hSetBuffering stdout NoBuffering
          ...
          putStrLn "Digite '1' para cadastrar um novo aluno"
          putStrLn "Digite '2' para sair do programa"
          op <- readLn
          putStrLn " "
          if op == 1
             then do putStrLn "Digite o nome completo do aluno: "
                     entradaNome <- getLine
                     putStrLn "Digite o numero de matricula: "
                     entradaNum <- getLine
                     putStrLn "Digite a nota: "
                     entradaNota <- getLine
                     putStr "Aluno "
                     putStr entradaNome
                     putStr " de numero de matricula "
                     putStr entradaNum
                     putStr " com nota "
                     putStr entradaNota
                     main
             ...
             then putStrLn "Saindo..."
		          ...
		          putStr "Opcao invalida, retornando ao menu...";
		          main;
